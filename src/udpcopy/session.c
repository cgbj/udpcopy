...
static time_t   send_begin_time = 0;

static uint64_t clt_udp_cnt      = 0;
static uint64_t clt_udp_send_cnt = 0;
static uint64_t qps_udp_send_cnt = 0;
...
bool process(char *packet, int pack_src)
{
    ...
    cur_time   = time(0);
    if (qps_udp_send_cnt == 0) {
        send_begin_time = time(0);
    }
    time_t  send_cost_time = cur_time - send_begin_time;
    //log_info(LOG_INFO, "qps_udp_send_cnt:%llu,send_cost_time:%llu,cur_time:%llu,send_begin_time=%llu",
    //                qps_udp_send_cnt, send_cost_time, cur_time, send_begin_time);
    if (qps_udp_send_cnt > send_cost_time * clt_settings.qps) {
        return true;
    }

    if (qps_udp_send_cnt > 100000000) {
        qps_udp_send_cnt = 0;
    } else {
        qps_udp_send_cnt++;
    }
